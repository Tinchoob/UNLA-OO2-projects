...
public class ClienteDao {
		...
		public Cliente traer(int dni)  throws HibernateException {
			...
			try {
				iniciaOperacion();
				objeto = (Cliente) session.createQuery("from Cliente as c where c.dni = "+dni).uniqueResult();
			}catch(HibernateException he) {
				manejaExcepcion(he);
				throw he;
			}
		...
		public List<Cliente> traer() throws HibernateException {
			...
			try {
				iniciaOperacion();
				lista = session.createQuery("from Cliente as c order by c.apellido asc").list();
		...
		public Cliente traerClienteYPrestamos(long idCliente) throws HibernateException{
			...
			try {
				iniciaOperacion();
				String hql = "from Cliente as c where c.idCliente = "+idCliente;		// el as???
